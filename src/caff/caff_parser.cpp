...
class CaffParser {
	...
	block_t parse_block(std::vector<uint8_t> raw_file_content, size_t& block_index) {
        ...
        if (block_index + new_block.length > raw_file_content.size()){
            LOG_ERROR(log) << "Block length too big!" << std::endl;
            exit(1);
        }
	...
	caff_header_t parse_caff_header(const std::vector<uint8_t>& data) {
        ...
        if (header.magic[0] != 'C' || header.magic[1] != 'A' || header.magic[2] != 'F' || header.magic[3] != 'F'){
            LOG_ERROR(log) << "Invalid magic!" << std::endl;
            exit(1);
        }
...
public:
	...
	caff_t parseCaff(std::string const &path_to_file) {
		...
		try {
            ...
            //A caff file must contain one header block
            //Thus the minimum file size is 29
            if(file_content.size() < 29){
                LOG_ERROR(log) << "Input file too small..." << std::endl;
                exit(1);
            }
